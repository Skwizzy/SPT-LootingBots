...
namespace LootingBotsServerMod
{
    ...
    public class PostDBLoad(DatabaseServer databaseServer, ConfigServer configServer, 
        JsonUtil jsonUtil, ModHelper modHelper, ISptLogger<PostDBLoad> logger) : IOnLoad
    {
        private readonly ConfigModel _config = jsonUtil.DeserializeFromFile<ConfigModel>(System.IO.Path.Join(
            modHelper.GetAbsolutePathToModFolder(Assembly.GetExecutingAssembly()), "config", "config.json")) ?? new();
